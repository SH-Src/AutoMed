...
from models_new.ablation_model import *
...
def main():
    ...
    parser.add_argument('-ab', '--ablation', default=1, type=int)
...
def train(args):
    ...
    if args.ablation == 4:
        ...
        model = Network4(pad_id + 1, args.d_model, args.steps, loss_func, None)
        ...
        print(genotype)
        if old_args.ablation == 1:
            model = Network1(pad_id + 1, old_args.d_model, old_args.steps, loss_func, genotype)
        elif old_args.ablation == 2:
            model = Network2(pad_id + 1, old_args.d_model, old_args.steps, loss_func, genotype)
        elif old_args.ablation == 3:
            model = Network3(pad_id + 1, old_args.d_model, old_args.steps, loss_func, genotype)
        elif old_args.ablation == 0:
            model = Network0(pad_id + 1, old_args.d_model, old_args.steps, loss_func, genotype)
        else:
