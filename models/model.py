...
from models.genotypes import STEPS
from models.utils import mask2d
from models.utils import LockedDropout
from models.utils import embedded_dropout
...
class DARTSCell(nn.Module):
  ...
  def forward(self, inputs):
    T, B, H = inputs.size(0), inputs.size(1), inputs.size(2)

    if self.training:
      x_mask = mask2d(B, inputs.size(2), keep_prob=1.-self.dropoutx)
      h_mask = mask2d(B, inputs.size(2), keep_prob=1.-self.dropouth)
    else:
      x_mask = h_mask = None

    hidden = inputs.new_zeros((B, H))
    hiddens = []
    for t in range(T):
