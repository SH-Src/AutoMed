...
class Network1(nn.Module):
    ...
    def new(self):
        model_new = Network1(self._vocab_size, self.d_model, self._steps, self._criterion).cuda()
...
class Network3(nn.Module):
    ...
    def new(self):
        model_new = Network3(self._vocab_size, self.d_model, self._steps, self._criterion).cuda()
        for x, y in zip(model_new.arch_parameters(), self.arch_parameters()):
            x.data.copy_(y.data)
        return model_new
...
class Network2(nn.Module):
    ...
    def new(self):
        model_new = Network2(self._vocab_size, self.d_model, self._steps, self._criterion).cuda()
        for x, y in zip(model_new.arch_parameters(), self.arch_parameters()):
            x.data.copy_(y.data)
        return model_new
    ...
    def forward(self, input_seqs, masks, lengths, seq_time_step):
        ...
        time_feature = 1 - self.tanh(torch.pow(self.selection_layer(seq_time_step), 2))
        time_feature = self.time_layer(time_feature)
        cat_feature = torch.cat((x, time_feature), dim=-1)
        fused_feature = self.catfc(cat_feature)
        alpha_fuse = torch.softmax(self.alphas_fuse, dim=-1)
        # alpha_fuse[-self._steps].data.copy_(F.one_hot(torch.tensor(4), num_classes=6).data)
        final_states = self.fuse_cell(fused_feature, masks, lengths, alpha_fuse)
...
class Network0(nn.Module):
    ...
    def new(self):
        model_new = Network0(self._vocab_size, self.d_model, self._steps, self._criterion).cuda()
        for x, y in zip(model_new.arch_parameters(), self.arch_parameters()):
            x.data.copy_(y.data)
        return model_new
    ...
    def forward(self, input_seqs, masks, lengths, seq_time_step):
        ...
        time_states = self.time_cell(time_feature, masks, lengths, torch.softmax(self.alphas_time, dim=-1))
        # alphas_select = torch.softmax(self.alphas_select, dim=-1)
        ehr_feature = sum(s for s in ehr_states) / len(ehr_states)
        time_feature = sum(s for s in time_states) / len(time_states)
        cat_feature = torch.cat((ehr_feature, time_feature), dim=-1)
        fused_feature = self.catfc(cat_feature)
        alpha_fuse = torch.softmax(self.alphas_fuse, dim=-1)
        # alpha_fuse[-self._steps].data.copy_(F.one_hot(torch.tensor(4), num_classes=6).data)
...
if __name__ == '__main__':
    ...
    model = Network2(10, 8, 2, loss_func)
